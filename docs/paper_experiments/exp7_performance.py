"""
docs/paper_experiments/exp7_performance.py
...
1. Matched-accuracy benchmark: wall-clock time to integrate the same
   ...
   and REBOUND (WHFast, timestep swept; IAS15 at its default settings --
   this installed REBOUND version (5.1.1) does not expose IAS15's adaptive
   epsilon parameter at the Python level, so IAS15 contributes one
   reference point rather than a full sweep). Timed with time.perf_counter
   around only the integration call itself (JIT warm-up for the Numba path
   is excluded -- see part 2's cold-vs-warm note).
...
def run_matched_accuracy():
    ...
    ax.loglog(e_, np.array(w) * 1e3, "o-", label="pyTIDES (TidesSolver)", color="tab:red")
